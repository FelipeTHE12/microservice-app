...
it("returns a 401 if the user does not own the ticket", async () => {
  const cookie = getCookieSignIn();
  console.log(cookie === getCookieSignIn());
  const title = "abcde";
  const price = 30;
...
it("returns a 400 if the user gives invalid title or price", async () => {
  const cookie = getCookieSignIn();
  const title = "abcde";
  const price = 30;
...
it("update ticket if the user gives valid title and price and also own the ticket", async () => {
  const cookie = getCookieSignIn();
  const title = "abcde";
  const price = 30;
